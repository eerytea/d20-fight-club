...
# ---- Auto caps & spell learning ----
try:
    from core.spell_catalog import SLOT_CAPS
except Exception:
    SLOT_CAPS = {}
try:
    from core.spell_training import learn_spells_for_level
except Exception:
    def learn_spells_for_level(f, level):
        return

def _cap_spell_slots_for_class(cls_name: str, pad_slots: list) -> list:
    caps = SLOT_CAPS.get(cls_name, {})
    capped = pad_slots[:]
    for lvl in range(0, min(10, len(capped))):
        cap = caps.get(lvl)
        if cap is not None:
            capped[lvl] = min(int(capped[lvl]), int(cap))
    return capped
...
def _apply_skald_casting_for_level(f: Dict[str, Any], L: int) -> None:
    ...
    pad = _cap_spell_slots_for_class(\"Skald\", pad)
pad = _cap_spell_slots_for_class(\"War Priest\", pad)
pad = _cap_spell_slots_for_class(\"Druid\", pad)
pad = [0] + slots
...
def _apply_wizard_init(f: Dict[str, Any]) -> None:
    ...
    pad = _cap_spell_slots_for_class(\"Wizard\", pad)
