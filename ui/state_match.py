...
class MatchState(BaseState):
    """Grid + HP on left, scrollable event log on right, Step/Auto/Finish controls."""
    ...
    def _fmt_event(self, e: Any) -> str:
        if _format_event:
            try: return _format_event(e)
            except Exception: pass

        if isinstance(e, dict):
            t = e.get("type") or e.get("event") or e.get("kind") or "event"

            def who_from(keys=("who","actor","src","i","a","idx","actor_idx","src_idx")):
                for k in keys:
                    if k in e: return self._name_from_val(e[k])
                return "?"

            def tgt_from(keys=("target","defender","dst","j","b","target_idx","defender_idx")):
                for k in keys:
                    if k in e: return self._name_from_val(e[k])
                return "?"

            if t in ("round","StartRound","start"):
                return f"— Round {e.get('round', e.get('r', '?'))} —"
            if t in ("move","Move"):
            ...
            try:
                parts = []
                for k, v in e.items():
                    if k in ("who","actor","src","i","a","idx","actor_idx","src_idx",
                             "target","defender","dst","j","b","target_idx","defender_idx"):
        ...
        # string fallback: try to substitute indices with names
        import re as _re
        s = str(e)
        def repl(m):
    ...
    def _build_match(self) -> None:
        ...
        # Start combat (signature-compatible)
        self.combat = TBCombat(tA, tB, fighters, _GRID_W, _GRID_H, seed=42)

        # If engine re-writes positions, fix obviously-bad layouts in place
        self._ensure_layout()
    ...
    def handle(self, event) -> None:
        if self.btn_step:   self.btn_step.handle(event)
    ...
    def update(self, dt: float) -> None:
        mx, my = pygame.mouse.get_pos()
        if self.btn_step:   self.btn_step.update((mx,my))
    ...
    def draw(self, surf: pygame.Surface) -> None:
        ...
        if self.btn_step:   self.btn_step.draw(surf, th)
    ...
    def _draw_grid(self, surf: pygame.Surface) -> None:
        ...
        for x in range(gw + 1):
            X = ox + x * cell_w
            pygame.draw.line(surf, self.theme.panel_border, (X, oy), (X, oy + gh * cell_h), 1)
        for y in range(gh + 1):
            Y = oy + y * cell_h
            pygame.draw.line(surf, self.theme.panel_border, (ox, Y), (ox + gw * cell_w, Y), 1)
