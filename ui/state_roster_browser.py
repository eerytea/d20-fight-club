...
class RosterBrowserState:
    """
    Two-pane browser:
      - Left: teams list (all league teams)
      - Right: selected team's roster (fighter name, class, level, OVR)
    Top-right: Generate button — clears & refills ALL rosters with fresh generated characters.
    Bottom-right: Back button — returns to Menu.
    """
    def __init__(self, app):
        self.app = app
        self.theme: Theme = Theme.default() if hasattr(Theme, "default") else Theme()
        self.screen = app.screen  # expect App to have .screen
    ...
    # ---------- Seeds & league generation ----------
    def _derive_seed(self, label: str) -> int:
        # Use app.derive_seed if present for determinism; otherwise fallback.
        if hasattr(self.app, "derive_seed") and callable(getattr(self.app, "derive_seed")):
            parent = getattr(self.app, "seed", DEFAULT_SEED)
            return int(self.app.derive_seed(f"roster:{label}:{self._seed_counter}"))
        # fallback: stable child of DEFAULT_SEED then add counter for variety
    ...
    def _regenerate(self):
        ...
        # Refresh list views with new content
        self._refresh_lists()
    ...
    def draw(self, surf):
        ...
        # Header / hint
        draw_text(surf, "Browse all teams and regenerate league rosters.", (16, 8), (200, 210, 220), 20)
